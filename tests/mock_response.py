...
from tests.helper__tst import NAME1, NAME3, SLASH_
...
class MockResponse(Response):
    ...
    exist_objects = [
        "api/v2/cmdb/firewall.schedule/onetime/{NAME1}",
    ...
    def get(cls, url: str) -> MockResponse:
        """session.get(), return data, status_code=200 if object is configured on Fortigate"""
        resp = cls(url=url)
        ...
        elif url_ in Action._known_urls:
            resp.status_code = 200
            resp._content = CONTENT_OBJECT
    ...
    def post(cls, url: str, **kwargs) -> MockResponse:
        ...
        if data_s := kwargs.get("data"):
            ...
            if data_d.get("name") in [NAME1, NAME3]:
                url_ = cls._url(url=url)
                if url_ in Action._known_urls:
                    resp.status_code = 200
        return resp

    @classmethod
    def delete(cls, url: str) -> MockResponse:
        """session.delete(), object is configured on firewall, status_code==200"""
        resp = cls(url=url)
        url_ = cls._url(url=url)
        if url_ in resp.exist_objects:
            resp.status_code = 200
    ...
    def put(cls, url: str, **kwargs) -> MockResponse:
        """session.put() (update), object is configured on firewall, status_code==200"""
        resp = cls(url=url)
        url_ = cls._url(url=url)
