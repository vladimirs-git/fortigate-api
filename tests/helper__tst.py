...
def address_get(url: str) -> Response:
    ...
    items = {
        ("A%2FB", ""): [{"name": "A/B"}],
        ("NAME1", ""): [{"name": "NAME1"}],
        ("NAME1", "filter=name%3D%3DNAME1"): [{"name": "NAME1"}],
        ("NAME2", ""): [],
        ("", "filter=name%3D%3DNAME1"): [{"name": "NAME1"}],
        ("", "filter=name%3D%3DNAME2"): [],
    }.get(key)
...
def address_post(url: str, data: dict) -> Response:
    ...
    uid = data["name"]
    status_code = {"NAME1": 500, "NAME2": 200, "NAME3": 200}[uid]
...
def address_put(url: str) -> Response:
    """Update."""
    uid = h.url_to_uid(url)
    status_code = {"NAME1": 200, "NAME3": 404}[uid]
...
def policy_post(url: str, data: dict) -> Response:
    data = json.loads(data["data"])
    uid = data["name"]
    status_code = {"NAME1": 500, "NAME2": 200}[uid]
    return create_response("post", url, status_code)
